...
import { fetchWakuwakuyaHtml } from "./wanoku-wakuwakuya-historical-audit.mjs";
import {
  buildWakuwakuyaEvidencePreview,
  validateWakuwakuyaExternalEvidenceInput
} from "./wanoku-wakuwakuya-evidence-preview.mjs";

export const WAKUWAKUYA_EVIDENCE_INGEST_VERSION = "wanoku-wakuwakuya-evidence-ingest.v1";
export const WAKUWAKUYA_PRODUCTION_WORKER_URL = "https://wanoku-intel-worker.mtk0808.workers.dev";

const SOURCE_MONTH = /^(20\d{2})-(0[1-9]|1[0-2])$/u;
const DEFAULT_DELAY_MS = 100;

export class WakuwakuyaEvidenceIngestError extends Error {
  constructor(code, message) {
    super(message);
    this.name = "WakuwakuyaEvidenceIngestError";
    this.code = code;
  }
}

export function parseWakuwakuyaEvidenceIngestArgs(argv = process.argv.slice(2)) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--month") {
      ...
      if (!value || value.startsWith("--")) {
        throw ingestError("missing_option_value", "Missing value for --month.");
      }
  ...
  if (!options.help && !options.month) {
    throw ingestError("month_required", "--month YYYY-MM is required.");
  }
  return options;
}

export function wakuwakuyaMonthlySourceUrl(value) {
  const month = requireSourceMonth(value);
  return `https://wakuwakuya.jp/blog.php?f=m&mon=${month}`;
}

export async function runWakuwakuyaEvidenceIngest(options = {}) {
  ...
  const workerBaseUrl = requireWorkerBaseUrl(options.workerBaseUrl ?? WAKUWAKUYA_PRODUCTION_WORKER_URL);
  const delayMs = requireDelay(options.delayMs ?? DEFAULT_DELAY_MS);
  const sourceUrl = wakuwakuyaMonthlySourceUrl(month);
  ...
  const buildPreview = options.buildPreviewImpl ?? buildWakuwakuyaEvidencePreview;
  const validateEvidence = options.validateEvidenceImpl ?? validateWakuwakuyaExternalEvidenceInput;
  const preview = buildPreview({ html, url: sourceUrl, sourceMonth: month, collectedAt: sourceFetchedAt });
  const summaryPath = options.summaryPath === undefined
    ? path.join(tmpdir(), `wanoku-wakuwakuya-ingest-${month}-summary.json`)
    : options.summaryPath;
  const summary = initialSummary({ apply, month, sourceFetchedAt, preview });
  const candidates = [];

  for (const article of preview.articles) {
    if (article.classification === "GENERATED") {
      const candidate = { article, record: evidenceRecord(article) };
      candidates.push(candidate);
      summary.records.push(candidate.record);
    } else {
      summary.records.push(skippedRecord(article));
    }
  }
  summary.records.push(...preview.parseErrors.map(parseErrorRecord));

  if (summary.identityCollision > 0) {
    addFatal(summary, "source_identity_collision", null, null);
  }

  const seenSourceIdentities = new Set();
  for (const candidate of candidates) {
    const validation = validateEvidence(candidate.article.externalEvidenceInput);
    if (!validPreparedCandidate(candidate.article, validation)) {
      addFatal(summary, "invalid_generated_evidence", candidate.article.evidenceId ?? null, null);
      continue;
    }
    if (seenSourceIdentities.has(validation.evidence.sourceIdentity)) {
      summary.identityCollision += 1;
    ...
    candidate.requestPayload = candidate.article.externalEvidenceInput;
...
async function createEvidence(fetchImpl, workerBaseUrl, adminSecret, candidate) {
  ...
  if (!validContract) {
    return { status: "FATAL", code: "create_response_contract_mismatch", httpStatus: response.status };
  }
...
function initialSummary({ apply, month, sourceFetchedAt, preview }) {
  const relevant = preview.articles;
  return {
    schemaVersion: WAKUWAKUYA_EVIDENCE_INGEST_VERSION,
    mode: apply ? "APPLY" : "DRY_RUN",
    month,
    sourceFetchedAt,
    collectedAt: preview.collectedAt,
    articlesDiscovered: preview.summary.articlesDiscovered,
    seabassRelevant: preview.summary.seabassRelevant,
    generated: preview.summary.evidenceGenerated,
    existing: 0,
    wouldCreate: 0,
    created: 0,
    skippedNotRepresentable: preview.summary.skippedNotRepresentable,
    skippedAmbiguous: preview.summary.skippedAmbiguous,
    cancelled: preview.summary.cancelled,
    stableIdUnresolved: relevant.filter((article) => article.sourceRecordId === null).length,
    identityCollision: relevant.filter((article) => (
      article.diagnostics.includes("source-identity-collision")
      || article.diagnostics.includes("source-event-identity-collision")
    )).length,
    failed: 0,
    fatalErrors: [],
    ...
    remaining: 0,
    evidenceTypeDistribution: { ...preview.summary.evidenceTypeDistribution },
    records: []
  };
}

function evidenceRecord(article) {
  const evidence = article.canonicalEvidence;
  return {
    sourceRecordId: article.sourceRecordId,
    sourceEventKey: article.sourceEventKey,
    evidenceType: evidence.evidenceType,
    presenceSupport: evidence.presenceSupport,
    catchCount: evidence.catchCount,
    interaction: { ...evidence.interaction },
    semanticClass: article.adapterMetadata.sourceSemanticClass,
    conditionChanges: article.adapterMetadata.conditionChanges.map((change) => ({ ...change })),
    habitatClues: [...article.adapterMetadata.habitatClues],
    diagnostics: [...article.diagnostics],
    evidenceId: article.evidenceId,
    status: "PENDING"
  };
}

function skippedRecord(article) {
  return {
    sourceRecordId: article.sourceRecordId,
    sourceEventKey: article.sourceEventKey,
    evidenceType: null,
    ...
    catchCount: null,
    interaction: null,
    semanticClass: article.adapterMetadata.sourceSemanticClass ?? null,
    conditionChanges: [],
    habitatClues: [],
    diagnostics: [...article.diagnostics],
    evidenceId: null,
    status: article.classification,
    reason: article.diagnostics.join(",")
  };
}

function parseErrorRecord(article) {
  return {
    sourceRecordId: article.sourceRecordId,
    sourceEventKey: article.sourceEventKey,
    evidenceType: null,
...
function validPreparedCandidate(article, validation) {
  return (
    !("sourceIdentity" in article.externalEvidenceInput)
    && !("adapterMetadata" in article.externalEvidenceInput)
    && validation?.valid === true
    && validation.evidence
    && validation.errors?.length === 0
    && validation.evidenceId === article.evidenceId
    && validation.semanticHash === article.semanticHash
    && validation.evidence.sourceIdentity === article.canonicalEvidence.sourceIdentity
  );
}
...
function ingestError(code, message) {
  return new WakuwakuyaEvidenceIngestError(code, message);
}

function printHelp() {
  console.log(`Usage:
  node scripts/wanoku-wakuwakuya-evidence-ingest.mjs --month YYYY-MM [--apply]
...
async function main() {
  const options = parseWakuwakuyaEvidenceIngestArgs();
  if (options.help) return printHelp();
  const summary = await runWakuwakuyaEvidenceIngest(options);
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  main().catch((error) => {
    console.error(`${error?.code ?? "wakuwakuya_evidence_ingest_failed"}: ${error?.message ?? "Wakuwakuya ingestion failed."}`);
    process.exitCode = 1;
  });
}
