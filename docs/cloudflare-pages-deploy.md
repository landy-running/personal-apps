...
この文書は、Cloudflare Pagesで公開済みのRunOS PWAとWanoku PWAの設定、確認手順、次フェーズ候補を記録する。legacy HTML、実データキー、IndexedDB、外部API、Cloudflare Workerにはまだ接続しない。
...
RunOS PWAとWanoku PWAは、2つのCloudflare Pagesプロジェクトとして分けて公開する。
