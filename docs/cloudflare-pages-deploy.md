...
この文書は、Cloudflare Pagesで公開済みのRunOS PWAとWanoku PWA、および短期本線として追加するRunOS Legacy PWAの設定、確認手順、次フェーズ候補を記録する。RunOS Legacy PWAは完成済みの `runos/RunOS v100 apex.html` をPWA配信するための構成であり、元HTML、実データキー、IndexedDB、外部API、Cloudflare Workerにはまだ接続しない。
...
- 破損JSON注入後、破損退避表示が出る
- RunOS Legacy PWAでは、旧HTML版の保存・復旧警告がPWA配信後も表示される
