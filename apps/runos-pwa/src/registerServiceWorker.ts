...
export function registerServiceWorker(onStatus?: ServiceWorkerStatusHandler): void {
  ...
  if (import.meta.env.DEV) {
    ...
    console.info("[runos-pwa] service worker registration skipped in development");
    navigator.serviceWorker
      .getRegistration(scopeUrl)
      .then((registration) => {
        if (!registration) return;
        return registration.unregister().then((removed) => {
          console.info("[runos-pwa] stale development service worker unregistered", removed);
        });
      })
      .catch((error: unknown) => {
        console.warn("[runos-pwa] stale service worker cleanup failed", error);
      });
    return;
  }
